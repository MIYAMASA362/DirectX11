...
void Font::Load(const char* fileName,const char* name)
{
	DeleteObject(this->m_font);
	DESIGNVECTOR design;
...
Texture* Font::TransformToTexture()
{
	...
	//テクスチャ生成
	{
		...
		hr =  D3DRenderer::GetInstance()->GetDevice()->CreateTexture2D(&desc, NULL, &texture);
		if (FAILED(hr))
			assert(false);
	}
	...
	//フォント情報をテクスチャに書き込み
	{
		...
		hr = D3DRenderer::GetInstance()->GetDeviceContext()->Map(
			texture,
			NULL,
	...
	D3DRenderer::GetInstance()->GetDeviceContext()->Unmap(texture, 0);
	...
	//SRV設定
	{
		...
		hr = D3DRenderer::GetInstance()->GetDevice()->CreateShaderResourceView(texture,&SRVDesc,&srv);
	}
