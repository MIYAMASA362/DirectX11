...
void CModel::Render()
{
	...
	{
		// マテリアル設定
		D3DApp::Renderer::SetMaterial( m_SubsetArray[i].Material.Material );

		// テクスチャ設定
		D3DApp::Renderer::SetTexture( m_SubsetArray[i].Material.Texture );
...
void CModel::Load( const char *FileName )
{
	...
	// 頂点バッファ生成
	{
		...
		D3DApp::GetDevice()->CreateBuffer( &bd, &sd, &m_VertexBuffer );
	}
	...
	// インデックスバッファ生成
	{
		...
		D3DApp::GetDevice()->CreateBuffer( &bd, &sd, &m_IndexBuffer );
	}
